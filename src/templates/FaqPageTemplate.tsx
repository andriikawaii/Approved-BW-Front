        ...
        <div className="mx-auto max-w-[1240px]">
          ...
          <div className="grid gap-8 lg:grid-cols-2">
            {(areas?.counties || []).map((county: any, index: number) => {
              ...
              const baseTowns = county.towns || [];
              const extraTowns = county.extra_towns || [];
              const links = county.town_links || {};
              return (
                <article key={`${county.name || "county"}-${index}`} className="area-card group flex flex-col overflow-hidden rounded-[12px] border border-[#e9e3d8] bg-white shadow-[0_2px_12px_rgba(30,43,67,0.06),0_1px_3px_rgba(30,43,67,0.04)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_14px_32px_rgba(30,43,67,0.1),0_28px_56px_rgba(30,43,67,0.12)]">
                  ...
                  <div className="flex flex-1 flex-col p-7 pb-8">
                    ...
                    <div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-4">
                      {baseTowns.map((town: string, townIndex: number) => (
                    ...
                    {expanded && extraTowns.length ? (
                      <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4">
                        {extraTowns.map((town: string, townIndex: number) => (
                          <span key={`${county.name || "county"}-extra-${town}-${townIndex}`} className="area-town-static block rounded-full bg-[#f5f1e9] px-3 py-2 text-center text-[11px] font-semibold text-[#1e2b43]">
                            {town}
                          </span>
                        ))}
                      </div>
      ...
      <section className="bg-[#f5f1e9] px-5 py-20 md:px-10" id="contact">
        <div className="mx-auto max-w-[1200px]">
          ...
          <div className="grid gap-8 lg:grid-cols-[1fr_1.08fr]">
            ...
            <div className="faq-contact-card rounded-[12px] border border-[#e4dac9] bg-white px-6 py-8 shadow-[0_20px_46px_rgba(30,43,67,0.1)] md:px-8">
              {submitted ? <div className="flex min-h-[420px] flex-col items-center justify-center text-center"><h3 className="text-[34px] font-bold">Thank You</h3><p className="mt-3 max-w-[420px] text-[15px] leading-7 text-[#5c677d]">We received your request and will get back to you within one business day.</p></div> : <form onSubmit={(event) => { event.preventDefault(); setSubmitted(true); }} className="faq-contact-form flex flex-col">
